...
export const RequestedAndInProgressIssues = (props: { issues: GitHubIssue[], search: string, showEmptyResult: boolean, votes: Tables<'voting'>[], userState: User | null, vote: (issueId: string, userState: User | null) => void }) => {
                                    ...
                                    <button onClick={() => props.vote(issue.id, props.userState)} className=" min-w-[3.5625rem] divide-solid flex flex-col items-center justify-center border border-solid border-[black] border-opacity-[38%] rounded overflow-hidden gap-[0.375rem] px-3 py-2">
                                        <svg width="16" height="9" viewBox="0 0 16 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M8.16636 0L0.878906 7.28745L2.59146 9L8.16636 3.43725L13.7413 9L15.4538 7.28745L8.16636 0Z"
                                                className={props.userState && props.votes.find(v => v.issueId == issue.id && v.userId === props.userState?.id) ? 'fill-primary' : 'fill-black'}
                                                fillOpacity={props.userState && props.votes.find(v => v.issueId == issue.id && v.userId === props.userState?.id) ? "0.85" : "0.38"} />
                                        </svg>
