...
const PiecesList = ({ pieces }: { pieces: PieceBase[] }) => {
  ...
  return (
    <div className="flex-col gap-4 flex justify-center items-center px-1 my-24">
      ...
      {!showAllResults && filteredPieces.length > 0 ? <button className=" container text-center text-xl transition-all hover:bg-primary hover:text-white py-5 w-full border border-solid border-primary rounded mb-5 text-primary " onClick={() => setShowAllResults(true)}>
        Load more
      </button> : null}
